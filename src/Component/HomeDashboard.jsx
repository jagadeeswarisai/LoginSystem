...
const HomeDashboard = () => {
  ...
  const backendUrl = "https://loginsystembackendecommercesite.onrender.com";

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [electronicsRes, appliancesRes] = await Promise.all([
          axios.get(`${backendUrl}/api/categories/bygroup/Electronics`),
          axios.get(`${backendUrl}/api/categories/bygroup/Home Appliances`),
        ]);

        setElectronicsCategories(electronicsRes.data);
        setApplianceCategories(appliancesRes.data);
        setLoading(false);
      } catch (err) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
        {categories.length > 0 ? (
          categories.map((cat) => (
            ...
            >
              <img
                src={cat.image ? `${backendUrl}/uploads/${cat.image}` : "/default-category.jpg"}
                alt={cat.name}
                className="w-40 h-40 object-cover rounded-lg mb-4 ring-2 ring-gray-300 mx-auto"
              />
  ...
  return (
    <div className="container mx-auto px-4 py-12 bg-gray-50 min-h-screen mt-10">
      {renderCategorySection("Best Deals on Electronics", electronicsCategories)}
      {renderCategorySection("Best Deals on Home Appliances", applianceCategories)}
    </div>
  );
};
