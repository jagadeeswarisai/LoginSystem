...
const HomeDashboard = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        // Filter categories by group "Home Appliances"
        const homeApplianceCategories = res.data.filter(
          (cat) => cat.group === "Home Appliances"
        );
        setCategories(homeApplianceCategories);
        setLoading(false);
      })
      .catch((err) => {
